...
async function fetchAirQuality(latitude: number, longitude: number, timezone: string): Promise<number | null> {
...
export async function fetchWeatherForLocation(location: GeoResult): Promise<WeatherData | null> {
  ...
  try {
    ...
    const airQuality = await fetchAirQuality(latitude, longitude, timezone || payload.timezone || 'auto');
